...
const EMPTY_FORM = { titulo: "", data: "", horaInicio: "", horaFim: "", descricao: "", categoria: "pessoal", alerta: false };
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="font-bold text-slate-900">{isEdit ? "Editar Evento" : "Novo Evento"}</h2>
        ...
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          <div>
            ...
            <input value={form.titulo} onChange={e => setForm(f => ({ ...f, titulo: e.target.value }))} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="time" value={form.horaFim} onChange={e => setForm(f => ({ ...f, horaFim: e.target.value }))} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
          </div>
          <div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.alerta} onChange={e => setForm(f => ({ ...f, alerta: e.target.checked }))} className="rounded" />
            <span className="text-sm text-slate-600">Ativar lembrete / alerta</span>
          </label>
        </div>
...
export default function EventosPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["agenda-events-all"] }); qc.invalidateQueries({ queryKey: ["agenda-events"] }); setShowNew(false); toast({ title: "Evento criado!" }); },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["agenda-events-all"] }); qc.invalidateQueries({ queryKey: ["agenda-events"] }); setEditing(null); toast({ title: "Evento atualizado!" }); },
  });
              ...
              return (
                <div key={e.id} className="flex items-start gap-4 p-4 bg-white rounded-xl border border-slate-200 hover:border-slate-300 transition-all group">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setEditing(e)} className="p-1.5 rounded-lg hover:bg-indigo-50 transition-colors">
